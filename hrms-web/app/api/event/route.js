...
function resolve(rule, actor, targetSid) {
  ...
  if (rule === "target") { const u = userBySubscriber(targetSid); return u ? [u] : []; }
  return [];
}
...
export async function POST(req) {
  ...
  const me = userBySubscriber(actor);
  if (!def || !me) return NextResponse.json({ error: "unknown action or actor" }, { status: 400 });
  const tenantName = (listTenants().find((t) => t.slug === me.tenant) || {}).name || me.tenant;
  const recipients = resolve(def.to, me, target).filter((r) => r.subscriberId !== me.subscriberId);
