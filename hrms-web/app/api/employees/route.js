...
export async function GET(req) {
  ...
  return NextResponse.json({ employees: (await employeesOf(tenant)).map(({ password, ...e }) => e) });
}
...
export async function POST(req) {
  try {
    ...
    const emp = await createEmployee({ tenant, name, email, password });
    const tenantName = ((await listTenants()).find((t) => t.slug === tenant) || {}).name || tenant;
    ...
    await notifyMany(await adminsOf(tenant), {
